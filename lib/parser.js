  ...
  * walk (tree, level) {
  ...
  parseNode (node) {
    if (node instanceof this.window.Text) return `| ${node.textContent}`
      ...
      if (node.childNodes.length === 1 && node.childNodes[0] instanceof this.window.Text) {
        line += `${this.addInlineText(line, node.childNodes[0].textContent)}`
      }
